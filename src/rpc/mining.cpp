...
static bool GenerateBlock(ChainstateManager& chainman, Miner& miner, CBlock& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    block_out.reset();
...
static UniValue generateBlocks(ChainstateManager& chainman, Miner& miner, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries)
{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            if (block.hashPrevBlock != miner.getTip()->GetBlockHash()) {
    ...
    result.pushKV("longpollid", miner.getTip()->GetBlockHash().GetHex() + ToString(nTransactionsUpdatedLast));
