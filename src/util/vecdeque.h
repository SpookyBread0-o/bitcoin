...
public:
    ...
    {
        if (m_size == m_capacity) Reallocate((m_size + 1) * 2);
        std::construct_at(m_buffer + Index(m_size), elem);
        ++m_size;
    }
    ...
    {
        if (m_size == m_capacity) Reallocate((m_size + 1) * 2);
        std::construct_at(m_buffer + Index(m_size), std::forward<Args>(args)...);
        ++m_size;
    }
    ...
    {
        if (m_size == m_capacity) Reallocate((m_size + 1) * 2);
        std::construct_at(m_buffer + Index(m_capacity - 1), std::move(elem));
    ...
    {
        if (m_size == m_capacity) Reallocate((m_size + 1) * 2);
        std::construct_at(m_buffer + Index(m_capacity - 1), elem);
        if (m_offset == 0) m_offset = m_capacity;
        --m_offset;
        ++m_size;
    }
    ...
    {
        if (m_size == m_capacity) Reallocate((m_size + 1) * 2);
        std::construct_at(m_buffer + Index(m_capacity - 1), std::forward<Args>(args)...);
        if (m_offset == 0) m_offset = m_capacity;
        --m_offset;
        ++m_size;
    }
